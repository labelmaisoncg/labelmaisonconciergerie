...
import type { LigneParc } from '../../../analyse';
...
@media print {
  ...
  .lm-impression { display: block; padding: 14mm; background: #fff; }
  .lm-impression article { border: 0 !important; padding: 0 !important; }
  @page { size: A4; margin: 0; }
}`;

/** La page remise au propriétaire (aperçu et impression). */
function DocumentProposition({ p, biens, choisies }: { p: Proprietaire; biens: LigneParc[]; choisies: RecommandationProprietaire[] }) {
  return (
    <article className="rounded-xl border border-(--lm-bord) bg-white p-5 text-[13px] text-(--lm-encre)">
      ...
      {biens.map(({ analyse: a }) => {
        const constats = a.defauts.filter((d) => d.source !== 'finance' && d.source !== 'mandat');
        return (
          <section key={a.logement.id} className="mb-3">
            <h3 className="font-semibold">{a.logement.nom}</h3>
            <p className="text-(--lm-encre-2)">
              Sur les 90 derniers jours : occupation {pourcentage(a.occupation90)}, prix moyen {euros(a.adr90, true)} la nuit, revenu brut{' '}
              {euros(a.f90.revenuBrut, true)}
              {a.note12 !== undefined ? `, note ${nombre(a.note12, 2)} / 5 (${a.nbAvis12} avis)` : ''}.
            </p>
            {constats.length > 0 && <p className="text-(--lm-encre-2)">Constats : {constats.map((d) => d.titre.toLowerCase()).join(', ')}.</p>}
          </section>
        );
      })}
            ...
            <tr key={r.id} className="border-b border-(--lm-bord) align-top">
              ...
              <td className="lm-chiffres py-1.5 text-right whitespace-nowrap">
                ...
                {r.impactSur && (
                  <>
                    <br />
                    <span className="text-[11px] text-(--lm-encre-3)">{LIBELLE_IMPACT[r.impactSur]}</span>
                  </>
                )}
              </td>
            </tr>
      ...
      <p className="mt-3 text-[11.5px] text-(--lm-encre-3)">
        Estimations indicatives calculées sur l’historique du bien ; elles ne constituent pas une garantie de revenu.
      </p>
 ...
 * améliorations retenues. Impression via window.print : seule la copie
 * `.lm-impression` (rendue dans body) sort sur papier.
 */
export function Proposition({ proprietaireInitial, onFermer }: { proprietaireInitial?: string; onFermer: () => void }) {
  ...
  const doc = p && choisies.length > 0 ? <DocumentProposition p={p} biens={biens} choisies={choisies} /> : null;
  ...
  return (
    <Modal
      ...
      pied={
        <>
          ...
          <Button icone={<Send />} disabled={!aProposer.length} onClick={() => aProposer.forEach((r) => upsert('recommandations', faireAvancer(r, 'proposee')))}>
            Marquer {aProposer.length || ''} comme proposée{aProposer.length > 1 ? 's' : ''}
          </Button>
          <Button variant="primary" icone={<Printer />} disabled={!doc} onClick={() => window.print()}>
            Imprimer
          </Button>
        </>
      }
    >
          ...
          <fieldset className="grid gap-1.5">
            ...
            {candidates.map((r) => (
              <label key={r.id} className="flex items-start gap-2 text-[13.5px]">
                ...
                <span>
                  {r.titre} <span className="text-(--lm-encre-3)">({r.statut === 'proposee' ? 'déjà proposée' : 'à proposer'})</span>
                </span>
              </label>
            ))}
          </fieldset>
      ...
      {doc && createPortal(<div className="lm-impression erp-root">{doc}</div>, document.body)}
    </Modal>
  );
}
