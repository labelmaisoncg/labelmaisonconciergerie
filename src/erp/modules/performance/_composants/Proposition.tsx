...
export function Proposition({
  ...
  const candidates = useMemo(
    () =>
      recommandations.filter(
        (r) =>
          r.proprietaireId === proprietaireId &&
          (r.statut === "a_proposer" || r.statut === "proposee"),
      ),
  ...
  const marquer = () => {
    for (const r of aProposer)
      upsert("recommandations", faireAvancer(r, "proposee"));
  };

  const document_ = p && (
    <article className="rounded-xl border border-(--lm-bord) bg-white p-5 text-[13px] text-(--lm-encre)">
      <header className="mb-4 flex items-start justify-between gap-4 border-b border-(--lm-or-anneau) pb-3">
        <div>
          ...
          <p className="text-[11.5px] text-(--lm-encre-2)">
            Conciergerie Airbnb en Essonne
          </p>
        </div>
        <p className="text-right text-[12px] text-(--lm-encre-2)">
          Le {dateJour(AUJOURDHUI)}
          <br />À l’attention de {p.nom}
        </p>
      </header>
      <h2 className="lm-serif mb-2 text-[18px]">
      ...
      {biens.map(({ analyse: a }) => (
        <section key={a.logement.id} className="mb-3">
          ...
          <p className="text-(--lm-encre-2)">
            Sur les 90 derniers jours : occupation {pourcentage(a.occupation90)}
            , prix moyen {euros(a.adr90, true)} la nuit, revenu brut{" "}
            {euros(a.f90.revenuBrut, true)}
            {a.note12 !== undefined
              ? `, note ${nombre(a.note12, 2)} / 5 (${a.nbAvis12} avis)`
              : ""}
            .
          </p>
          {a.defauts.filter(
            (d) => d.source !== "finance" && d.source !== "mandat",
          ).length > 0 && (
            <p className="text-(--lm-encre-2)">
              Constats :{" "}
              {a.defauts
                .filter((d) => d.source !== "finance" && d.source !== "mandat")
                .map((d) => d.titre.toLowerCase())
                .join(", ")}
              .
            </p>
          )}
          ...
          {choisies.map((r) => (
            <tr key={r.id} className="border-b border-(--lm-bord) align-top">
              ...
              <td className="py-1.5 pr-2 whitespace-nowrap">
                {LIBELLE_PORTEUR[r.porteur]}
              </td>
              <td className="lm-chiffres py-1.5 text-right whitespace-nowrap">
                {r.impactEstimeCentimesMois !== undefined
                  ? `+${euros(r.impactEstimeCentimesMois, true)} / mois`
                  : "Qualité"}
                {r.impactSur && (
                  <>
                    <br />
                    <span className="text-[11px] text-(--lm-encre-3)">
                      {LIBELLE_IMPACT[r.impactSur]}
                    </span>
                  </>
                )}
              </td>
      ...
      <p className="mt-3 text-[11.5px] text-(--lm-encre-3)">
        Estimations indicatives calculées sur l’historique du bien ; elles ne
        constituent pas une garantie de revenu.
      </p>
      <footer className="mt-6 flex items-end justify-between gap-4">
        <p className="text-[12px] text-(--lm-encre-2)">
          Bon pour accord, date et signature du propriétaire :<br />
          <br />
          ........................................
        </p>
  ...
  return (
    <Modal
      ...
      pied={
        <>
          ...
          <Button
            icone={<Send />}
            disabled={!aProposer.length}
            onClick={marquer}
          >
            Marquer {aProposer.length || ""} comme proposée
            {aProposer.length > 1 ? "s" : ""}
          </Button>
          <Button
            variant="primary"
            icone={<Printer />}
    ...
    >
      <style>{`.lm-impression { display: none; }
      @media print {
        body > *:not(.lm-impression) { display: none !important; }
        .lm-impression { display: block; padding: 14mm; }
        .lm-impression article { border: 0 !important; }
      ...
      <div className="grid gap-4">
        ...
        {proprietaireId && !candidates.length && (
          <Alert tone="info">
            Aucune amélioration à proposer ou en cours de proposition pour ce
            propriétaire.
          </Alert>
        )}
        {candidates.length > 0 && (
          <fieldset className="grid gap-1.5">
            <legend className="mb-1 text-[13px] font-medium text-(--lm-encre)">
            ...
            {candidates.map((r) => (
              ...
              >
                <input
                  type="checkbox"
                  className="mt-1 accent-(--lm-or)"
                  checked={coches ? coches.has(r.id) : true}
                  onChange={() => basculer(r.id)}
                />
                <span>
                  {r.titre}{" "}
                  <span className="text-(--lm-encre-3)">
                    ({r.statut === "proposee" ? "déjà proposée" : "à proposer"})
                  </span>
                </span>
              </label>
            ))}
        ...
        {p && choisies.length > 0 && document_}
      </div>
      {p &&
        choisies.length > 0 &&
        createPortal(
          <div className="lm-impression erp-root">{document_}</div>,
          document.body,
        )}
    </Modal>
  );
